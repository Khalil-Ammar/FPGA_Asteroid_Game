...
//////    			left = (int)left * volume/4;
//////    			right = (int)right * volume/4;
//////
//////    			//prevent clipping
//////    			left = left > MAX_AMP ? MAX_AMP : left;
//////    			left = left < MIN_AMP ? MIN_AMP : left;
//////    			right = right > MAX_AMP ? MAX_AMP : right;
//////    			right = right < MIN_AMP ? MIN_AMP : right;
////
...
//			if (res != FR_OK || finf.fname[0] == 0) {
//				break;
//			}
//			if (finf.fattrib & AM_DIR) {                 // It's a directory
//			}
//			else if (strstr(finf.fname,".wav")!=NULL || strstr(finf.fname,".WAV")!=NULL) { // It's a WAV file
...
//
//		if (!fileFound) {
//			print("WAV file not found. Press RETURN to try again\r\n");
//			getchar();
//			continue;
//		}
//
    ...
    if (audioBuffer) {
    	...
    	// Changing the volume and swap left/right channel and polarity
    	{
    		...
    		for(u32 i=0;i<audioBufferSize/4;++i) {
				...
				int left_i  = -(int)left * volume / 4;
				int right_i = -(int)right * volume / 4;
				//prevent clipping
    			left_i = left_i > MAX_AMP ? MAX_AMP : left_i;
    			left_i = left_i < MIN_AMP ? MIN_AMP : left_i;
    			right_i = right_i > MAX_AMP ? MAX_AMP : right_i;
    			right_i = right_i < MIN_AMP ? MIN_AMP : right_i;
				left = (short)left_i;
				right = (short)right_i;
				pSource[i] = ((u32)right<<16) + (u32)left;
    		}
		...
		for(;;) {
			...
			if(c == 112) {		// p
				...
				continue;
							...
							volume++;
							volume = volume > 8 ? 8 : volume; // 200% max
							playWavFile(fileName);
							continue;
						}
			else if(c == 100){	// d
