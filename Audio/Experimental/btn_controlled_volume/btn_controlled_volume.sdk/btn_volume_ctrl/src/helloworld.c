...
#define INTC_DMA_INTERRUPT_ID	XPAR_FABRIC_AXIDMA_0_VEC_ID

#define BTN_INT 				XGPIO_IR_CH1_MASK

#define BASE_VOLUME				16384
...
static int volume = BASE_VOLUME;
static u32 buffer[PERIODSAMPLES];
...
void generate_sound(double volume){
	double amp = volume;
	for(int i=0;i<128;++i) {
		short left = (short) (cos((double)i/128*2*M_PI) * amp);
		short right = (short) (sin((double)i/128*2*M_PI) * amp);
		buffer[i] = ((u32)left<<16) + (u32)right;
	}
}

//----------------------------------------------------
// BUTTON INTERRUPT PROTOTYPE FUNCTIONS
//----------------------------------------------------
...
void BTN_Intr_Handler(void *InstancePtr) {
	...
	if (btn_value == 0b10000){
		volume *= 1.1;
		generate_sound(volume);
	}

	//Down button decreases volume by 10%
	else if (btn_value == 0b00010){
		volume *= 0.9;
		generate_sound(volume);
	}

	//Center button resets volume
	else if (btn_value == 0b00001){
		volume = BASE_VOLUME;
		generate_sound(volume);
	}
...
int main()
{
    ...
    adau1761_init(&codec, XPAR_AXI_FIFO_MM_S_0_DEVICE_ID, XPAR_AXI_DMA_0_DEVICE_ID, INTC_DMA_INTERRUPT_ID);
